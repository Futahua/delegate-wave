...
// returns the DISPLAY projection, which is not the producer's evidence API. A routed wake is now a
// typed timeline row (`display_kind="delegate_wave_wake"`), and the canonical projection preserves
// that durable type even when presentation rules evolve. `session.canonical_history` is the
// evidence projection: the durable lineage and its display kinds.
//
